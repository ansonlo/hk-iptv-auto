...
# 🌟 重點：確保 auto_repair.log 被正確創建並追加
...
def diagnostic_and_test(source_list, ip_cache):
    ...
    for p in ["移动", "电信", "联通", "广电"]:
        ...
        log_info(f"🛰️ 診斷中: 【{p}】模式 (Mask: {mask})")
        
        for u in source_list:
            try:
                r = test_session.get(u, timeout=8, headers=headers, verify=False)
                r.encoding = 'utf-8'
                name, raw_items = "", []
                        ...
                        for f in futures:
                            ...
                            if s < 5.0:
                                if any(k in item['name'].upper() for k in KEYWORDS):
                                    ...
                                    new_item = item.copy(); new_item['speed'] = final_s
                                    provider_items[p].append(new_item)
                                    source_stats[u][p] += 1
            except: continue

    log_info(f"🌐 診斷中: 【通用模式】測試...")
...
def main():
    ...
    valid_p = {k: v for k, v in all_provider_final_data.items() if v}
    best_p_key = max(valid_p, key=lambda k: len(valid_p[k])) if valid_p else None
